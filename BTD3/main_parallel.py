...
if __name__ == "__main__":
    ...
    parser.add_argument('--env-name', default="Walker2d-v3", type=str, help='Name of a environment (set it to any Continous environment you want')
    ...
    parser.add_argument('--save_model', default=False, type=bool)        # Boolean checker whether or not to save the pre-trained model
    parser.add_argument('--lr_critic', default=1e-2, type=float)        # learning rate critic
    parser.add_argument('--lr_actor', default=1e-3, type=float)         # learning rate actor
    ...
    parser.add_argument('--repeat', default=2, type=int)           # Number of iterations to wait before the policy network (Actor model) is updated
    parser.add_argument('--target_update_freq', default=50, type=int)  # steps between the hard target upate for the critics
